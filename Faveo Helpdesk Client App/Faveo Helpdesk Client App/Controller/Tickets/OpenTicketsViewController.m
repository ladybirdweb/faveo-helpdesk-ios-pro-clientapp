...
@interface OpenTicketsViewController ()<RMessageProtocol>
{
...
@implementation OpenTicketsViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    
    [self setTitle:NSLocalizedString(@"Open Tickets",nil)];
    ...
    {   NSString *msg=@"";
          [utils showAlertWithMessage:@"API is disabled in web, Contact to Faveo Support." sendViewController:self];
        [self->userDefaults setObject:msg forKey:@"msgFromRefreshToken"];
...
-(void)reload{
    ...
    if ([[Reachability reachabilityForInternetConnection]currentReachabilityStatus]==NotReachable)
    {
        ...
        NSString * url= [NSString stringWithFormat:@"%@api/v1/helpdesk/user/ticket/list?token=%@&status=%@",[userDefaults objectForKey:@"baseURL"],[userDefaults objectForKey:@"token"],@"open"];
        NSLog(@"URL is : %@",url);
            ...
            [webservices httpResponseGET:url parameter:@"" callbackHandler:^(NSError *error,id json,NSString* msg) {
                ...
                if (error || [msg containsString:@"Error"]) {
                    ...
                    if (msg) {
                        ...
                        NSLog(@"Thread-openTickets-Refresh-error == %@",error.localizedDescription);
                ...
                if (json) {
                    ...
                    self->globalVariables.openTicketsCount =count;
...
//                    self->_totalPages=[[data1Dict objectForKey:@"last_page"] integerValue];
                ...
                NSLog(@"Thread-openTickets-closed");
                }
            }];
        ...
        @finally
        {
            NSLog( @" I am in reload method in Open Tickets ViewController" );
...
//            }
            ...
            if([NSNull null] != [finaldic objectForKey:@"assignee"]) {
                ...
                if (![Utils isEmpty:assigneeFirstName] || ![Utils isEmpty:assigneeLaststName])
                {
                    if  (![Utils isEmpty:assigneeFirstName] && ![Utils isEmpty:assigneeLaststName])
                    {
                        cell.mailIdLabel.text=[NSString stringWithFormat:@"Agent: %@ %@",assigneeFirstName,assigneeLaststName];
                    }
                    else
                    {
                        cell.mailIdLabel.text=[NSString stringWithFormat:@"Agent: %@ %@",assigneeFirstName,assigneeLaststName];
                ...
                {
                    cell.mailIdLabel.text= assigneeUserName;
                }
                ...
                cell.mailIdLabel.text= NSLocalizedString(@"Unassigned", nil);
...
//            }
        ...
        @finally
        {
            NSLog( @" I am in cellForAtIndexPath method in Opent tickets ViewController" );
