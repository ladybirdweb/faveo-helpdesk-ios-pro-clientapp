...
@interface ConversationViewController ()<ConversationTableViewCellDelegate,UIWebViewDelegate,RMessageProtocol>{
    ...
    OpenTicketsViewController * openTicketsPage;
}
...
- (void)viewDidLoad {
    ...
    openTicketsPage=[[OpenTicketsViewController alloc]init];
    ...
    [SVProgressHUD setDefaultMaskType:SVProgressHUDMaskTypeBlack];
     ...
     [SVProgressHUD showWithStatus:@"Loading Conversations"];
       [self reload];
...
-(void)reload{
    ...
    if ([[Reachability reachabilityForInternetConnection]currentReachabilityStatus]==NotReachable)
    {
        ...
        NSString *url=[NSString stringWithFormat:@"%@helpdesk/ticket-thread?api_key=%@&ip=%@&token=%@&id=%@",[userDefaults objectForKey:@"companyURL"],API_KEY,IP,[userDefaults objectForKey:@"token"],globalVariable.ticket_id];
...
//
    ...
    if([NSNull null] != [finaldic objectForKey:@"user"])
    {
        ...
        {
            NSString *name = [fName substringToIndex:2];
            [cell.profilePicView setImageWithString:name color:nil ];
        }
